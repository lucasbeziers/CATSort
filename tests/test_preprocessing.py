...
def test_preprocess_recording_no_op(dummy_recording):
    ...
    processed = preprocess_recording(dummy_recording, normalize=False, freq_min=None, freq_max=None, whitening=False)
...
def test_preprocess_recording_full(dummy_recording):
    ...
    processed = preprocess_recording(dummy_recording, normalize=True, freq_min=300, freq_max=3000, whitening=True)
    assert processed is not None
    assert processed.get_num_channels() == dummy_recording.get_num_channels()
...
def test_preprocess_recording_individual_steps(dummy_recording):
    ...
    # Only normalization
    processed_norm = preprocess_recording(
        dummy_recording, normalize=True, freq_min=None, freq_max=None, whitening=False
    )
    ...
    # Only whitening
    processed_white = preprocess_recording(
        dummy_recording, normalize=False, freq_min=None, freq_max=None, whitening=True
    )
