...
def compute_fixed_thresholds(features: dict, mad_multipliers: dict) -> dict:
    """
...
def compute_collision_features(
    peaks_traces: np.ndarray, sampling_frequency: float, width_threshold_amplitude: float = 0.5
) -> dict:
    """
    ...
    return {"amplitude": amplitudes, "width": widths_ms, "energy": energies}
...
def optimize_collision_thresholds(
    features: dict, temporal_collisions: np.ndarray, false_positive_tolerance: float = 0.05
) -> dict:
    """
